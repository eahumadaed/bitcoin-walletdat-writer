...
#
# --- vendored into walletdat-forge ---
# Unmodified except for the import below: upstream lives at
# https://github.com/bitcoin/bitcoin/blob/master/test/functional/test_framework/key.py
# as part of the `test_framework` package (`from test_framework.crypto import
# secp256k1`). Here it's used as a standalone file, so the import is
# rewritten to a flat `import secp256k1`, and the test-only helpers from
# `test_framework.util` (only used by the unittest class at the bottom of
# this file, not by anything walletdat-forge calls) get a local fallback
# instead.
...
import secp256k1  # patched: flat file instead of the test_framework.crypto package
try:
    from test_framework.util import assert_not_equal, random_bitflip
except ImportError:
    pass  # only used by the TestFrameworkKey unittest class below, not needed here
